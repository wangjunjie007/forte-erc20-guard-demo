...
export function resolveTemplatePath(template: string): string {
  ...
  throw new Error(
    `Template not found: ${template}. Available templates: ${available.length ? available.join(", ") : "<none>"}`,
  );
}
...
export class FortePolicyHelper {
  ...
  static async create(rawConfig: {
    ...
    const config: PolicyHelperConfig = {
      ...
      tokenAddress: ensureHexAddress(rawConfig.tokenAddress, "tokenAddress"),
    };
    ...
    return new FortePolicyHelper(config, rulesEngine, wagmiConfig);
  }
  ...
  async createAndApplyTemplate(template: string, options?: { apply?: boolean }): Promise<CreateApplyResult> {
    ...
    const appliedPolicyIdsRaw = await this.rulesEngine.getAppliedPolicyIds(this.config.tokenAddress);
